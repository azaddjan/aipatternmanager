...
import {
  ...
  aiDocumentSectionAssist, exportDocumentDocx,
} from '../api/client'
  ...
  const handleSave = async () => {
    ...
    try {
      if (isNew) {
        ...
        await updateDocument(id, { title, doc_type: docType, status, summary, target_audience: targetAudience, tags, team_id: teamId || null, updated_by: user?.name || user?.email || '' })
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {!isNew && !editMode && (
            <>
              <button onClick={handleExportDocx} className="btn-secondary text-sm flex items-center gap-1.5">
